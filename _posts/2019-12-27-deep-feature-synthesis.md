...
피처를림 1. 전형적인 데이터 과피처노력. 분석가는 질 문을 제기하면서 시작합니다: x 또는 y가 z와 상관되어 있는지 예측할 수 있습니까? 이러한 질문은 일반적으로 데이터를 보유한 비즈니스 또는 연구원의 요구에 따라 정의됩니다. 어는 설명 변수를 가정하고 해당 변수를 추출하는 스크립트를 작성합니다. 이러한 변수 / 피처 및 피처를 고려할 때 기계 습 피처와은 주어진 예측 목표에 대한 모델을 작성하고, 다양한 모델링 기술을 적용합다. 현재  예측 문제에 대한 가장 일반적인 방법을 식 별하기 위해 모델링 방법의 공간 내에서 선택을 합니다. 데이터 학자특성모델을 작성, 개,선 및 증하기 위해 질문을 가정하고 변수를 형성하는 전체 과정을 수행할 수 있습니다.
...
사용 중에 데터 피처와머신은 필요할 때만 데이터베이스 쿼리를 만듭니다. 피처를 계산하는데 필요한 데이터는 여러 테이블에 저장될 수 있으므로 쿼의 `SE LECT` 절 또는 `WHERE` 절에 있는  모든 열에 액세스 할 수 있도록 테이블을 조인해야 합니다. 쿼리 예는그림특성에 있습니다. 쿼,리는 조과 테이블 스캔 수를 줄이기 위해 한 번에 여러 계산을 수행하도록 피처를되었습니다.
