...
Snakemake 7.8 버전 이후로는 결과 파일을 생성한 당시의 패러미터나 소스코드, 입력 파일등이 바뀌었을 때 자동으로 빌드하도록 재실행 정책이 바뀌었다. 

https://github.com/snakemake/snakemake/issues/1694

필자의 경우 예전 버전을 사용하면서 코드나 패러미터가 변경되었을 때 자동으로 재실행되지 않아 명시적으로 -f 옵션을 주어 빌드하곤 했었다. 이번 정책 변경으로 좀 더 스마트하게 Snakemake 를 활용할 수 있을 듯 하다. 

그렇지만, 만약 이런 정책이 맞지 않은 경우 `--rerun-triggers=mtime` 옵션을 주어 과거 버전과 같게 동작하도록 설정할 수 있다.
