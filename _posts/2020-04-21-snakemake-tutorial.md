...
데이터 분석가 (u2) 는 데이터 엔지니어 (u1) 의 워크 플로우를 `include` 하고, 그것의 최종 타겟인 `../temp/wc_all.csv` 를 입력으로 하여 `plot` 규칙을 정의하고 있다. 기본 규칙은 include 된 파일의 것은 무시하고, 현재 워크플로우의 첫 규칙인 `u2all` 이 선택된다.
