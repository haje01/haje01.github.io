...
- 누락이나 오류가 있을 수 있으니, 정확한 내용은 꼭 원문을 참고하시기 바란다.

---

## 도입

* 파이썬 패키지 [TPOT (Tre-based Pipeline Optimization Tool)](https://github.com/EpistasisLab/tpot) 의 기반 논문
* 최근 데이터와 분석 수요의 급격한 증가에 따라, 비전문가의 분석을 지원하기 위해 확장 가능하고 융통성있는 툴이 필요
* 기존 ML 툴은 최적의 데이터 파이프라인을 찾기 위해 많은 사전 지식과 시도를 필요로 하기에 비용이 큼
...
>
> 데이터 모델을 찾기 전
> - 초기 탐색적 분석 - 누락되거나 잘못 레이블이 지정된 데이터 찾기를 수행하여 모델링 할 데이터를 준비
> - 데이터 정리 - 문제가 있는 기록을 수정하거나 제거
> - 피처 전처리- 피처 정규화 등
> - 피처 선택 - 모델링에 유용하지 않은 피쳐를 제거
> - 피처 구축 - 기존 데이터에서 새 피처를 만듦
>
> 등의 방법으로 모델링에보다 적합한 방식으로 데이터를 한다. 그 후,
> - 모델 선택 - 데이터에 맞는 머신러닝 모델을 선택
> - 매개 변수 최적화 - 모델이 데이터에서 가장 정확한 분류를 수행할 수 있도록 모델 매개 변수를 선택
> - 모델 유효성 검증 - 제외된 홀드 아웃 데이터 셋에서 모델의 성능을 테스트하여, 모델 예측이 수정되지 않은 데이터 셋에 일반화되도록 검증
>
> 파이프 라인의 초기 단계에서 밝은 회색 영역은 TPOT (Tree-based Pipeline Optimization Tool)에 의해 자동화되는 파이프 라인의 단계를 나타낸다.
...
* 우리 연구는 진화 알고리즘에 기반한 *트리 기반 파이프라인 최적화 툴(Tree-based Pipeline Optimization Tool, TPOT)* 에 관한 것
  * TPOT 은 *유전자 프로그래밍 (Genetic Programming)* 을 이용해, 데이터 변환 및 머신러닝 모델을 최적화


## 관련 연구
- 지금까지의 머신러닝 자동화는 파이프라인 일부의 최적화에 집중
  - 매개 변수 최적화 (Hyper-parameter Optimization)
  - 피처 구축 (Feature Construction) - [심층 피처 합성](https://haje01.github.io/2019/12/27/deep-feature-synthesis.html) 을 활용한 *데이터 과학 머신*

- 최근, Fuerer 등은 베이지안 최적화를 사용하여 피처 전처리, 모델 및 모델 매개 변수의 최적 조합을 찾아주는 [auto-sklearn](https://automl.github.io/auto-sklearn/master/) 을 개발
  - 그러나, 하나의 데이터 전처리, 피처 전처리 그리고 모델만을 포함하는 파이프라인 셋를 탐색
...
TPOT 에 구현된 네 가지 파이프라인 연산자를 소개. 모든 연산자는 [scikit-learn](https://scikit-learn.org/stable/) 을 사용해 구현
...
* 모든 연산자들을 결합하는 유연한 파이프라인 구조를 위해 그림 2. 처럼 파이프라인을 트리로 구현.

![그림 2](/assets/2020-01-13-13-58-12.png)

> 그림 2. 트리 기반 머신러닝 파이프라인의 예. 데이터 셋은 피처를 연속적으로 더하고, 제거하고, 수정하는 파이프라인 연산자를 통과. 결합 연산자는 개별 데이터 카피본을 결합하고, 최종 분류를 위해 분류기에 제공.
...
* 데이터는 계층화된 훈련 셋 75%와, 테스트 셋 25%로 나눔

* 트리기반 파이프라인은 임의의 파이프라인 표현이 가능

### 진화하는 트리기반 파이프라인

* 트리기반 파이프라인 생성 및 최적화에 파이썬 패키지  [DEAP](https://deap.readthedocs.io/en/master/) 으로 구현된 *유전 프로그래밍 (Genetic Programming)* 을 사용
* 파이프라인 연산자 시퀀스와 각 연산자의 매개 변수를 모두 GP로 진화
* 표 1의 설정으로 표준 GP 과정을 따름
...
* GP는 진화의 세대마다 파이프라인을 조금씩 수정
  * 최종적으로 가장 우수한 파이프라인을 선택

### GAMETES 시뮬레이션 데이터 셋

* GAMETES 은 순수하고 엄격한 상위 유전자 모델을 생성하기 위한 오픈소스 패키지
* 이를 이용해 12 가지 유전자 모델 및 360 개 연관 데이터 셋으로 TPOP 을 평가

### UCI 벤치마크 데이터 셋

* 잘 알려진 [UC-Irvine Machine Learning Repository](https://archive.ics.uci.edu/ml/index.php) 의 9 가지 지도학습 데이터 셋으로 TPOT을 평가.
* 이것은 다양한 범위의 활용 영역에서 TPOT 의 성능을 보여주기 위한 것
    * *Hill-Valley* : 지형 데이터에서 언덕과 계곡을 예측
    * *breast-cancder-wisconsin* : 유방암 여부를 예측
    * *car-evaluation* : 네 가지 자동차 구매 허용도를 예측
    * *glass* : 다양한 풀의 데이터에서 종류를 예측
    * *ionosphere* : 고주파 안테나의 불량을 예측
    * *spambase* : 이메일의 단어 빈도에서 스팸 예측
    * *wine-quality* : 적/백 포도주의 11단계 품질 예측

## 결과

TPOT 의 분류 성능을 두 컨트롤과 비교

* 첫 번째는 500 개 결정 트리를 가진 랜덤 포리스트
* 두 번째는 *TPOT Random*
  * 같은 수의 파이프라인이 임의로 생성됨
  * 유도된 (guided) 검색이 파이프라인 최적화에 유용한지 확인하기 위함
* 추가적으로 *TPOT Pareto* 와 비교
  * 우수하면서 가장 작은 파이프라인을 찾기위해 파레토 최적화를 이용
...
> * 각 노치 박스는 30 데이터 셋의 표본에 해당 (노치는 중간값의 95% 신뢰 구간)
> * 4 종류 비교
>   * *Random Forest* : 500 결정 트리의 랜덤 포리스트
...
> * 우상단 (높은 유전 가능성 모델에서 많은 데이터 셋이 생성) 이 쉽고 , 좌하단 (낮은 유전 가능성 모델에서 작은 데이터 셋이 생성) 으로 갈수록 어려움

* RF는 가장 쉬운 경우 (샘플 크기: 1660, 유전 가능성: 0.4) 에서도 특성간 상위 상호 작용 발견에 실패했고, 평균 63% 정확도
* 반변 모든 버전의 TPOT 은 80% 이상의 정확도
...
* GAMETES 에서와 비슷하게 *TPOT-Random* 은 유도된 TPOT 과 비슷한 성능
...
* 지금까지 자동화된 머신러닝 파이프라인 설계 및 최적화가, 사전 지식 없이도 기초적 머신 러닝에 비해 큰 향상을 제공할 수 있다는 것을 보임
...
* 우리는 가까운 미래에 *auto-sklearn* 또는 다른 휴리스틱을 이용해 유망한 파이프라인 모집단을 구성하도록 시도할 계획
