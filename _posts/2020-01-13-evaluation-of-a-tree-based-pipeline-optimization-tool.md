...
* 모든 연산자들을 결합하는 유연한 파이프라인 구조를 위해 그림 2. 처럼 파이프라인을 *트리* 로 구현

![그림 2](/assets/2020-01-13-13-58-12.png)

> 그림 2. 트리 기반 머신러닝 파이프라인의 예. 데이터 셋은 피처를 연속적으로 더하고, 제거하고, 수정하는 파이프라인 연산자를 통과. 결합 연산자 (Combine Operator) 는 개별 데이터 카피본을 결합하고, 최종 분류를 위해 분류기에 제공
