...
여기에서 `token=` 이후의 토큰값을 기록해둔다. ECS 대쉬보드에서 스케쥴러와 같은 식으로 `dask-notebook` 작업의 퍼블릭 IP를 찾은 뒤 아래와 같이 `8888` 포트로 접속해보자.
...
* 만약 Jupyter 노트북은 로컬 PC에 설치된 것을 사용하고, Dask 스케쥴러와 워커만 도커 이미지를 이용해 클라우드에 띄우고자 한다면, 상호간 설치된 패키지(특히 Dask 와 s3fs)의 버전을 최대한 맞춰주는 것이 좋다.
