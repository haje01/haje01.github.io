...
---
{% raw %}
컨테이너화와 쿠버네티스는 최근 IT 업계에서 매우 중요한 기술 중 하나이다. 컨테이너화는 애플리케이션을 빠르고 쉽게 배포할 수 있도록 해주며, 컨테이너 오케스트레이션 툴인 쿠버네티스는 컨테이너를 쉽게 관리하고 스케일링하는 등의 기능을 제공한다.

ETL(Extract, Transform, Load)은 데이터를 추출하고, 변환하여 필요한 형식으로 가공한 다음, 로드하여 저장소에 저장하는 과정을 말한다. 그렇지만 이 글은 본격적인 ETL 기술을 다루는 것이 아니라, ETL 예제를 통해 쿠버네티스 환경에서 Argo Events 와 Workflows 의 활용 방법을 보예주는 것에 초점17을 맞춘다.

[Argo Events](https://argoproj.github.io/argo-events/) 는 쿠버네티스 환경에서 이벤트 기반 워크플로우 자동화를 지원하는 툴이다. 다양한 이벤트 소스로부터 이벤트를 감지하고, 다음 워크플로우를 실행해준다. [Argo Workflows](https://argoproj.github.io/argo-workflows/) 는 쿠버네티스 기반 워크플로우 엔진으로, ETL 1과정의 각 단계를 효율적으로 처리할 수 있도록 도와준다. 둘을 결합하면 데이터 감지 및 처리 과정을 자동화하고, 빠르고 정확하게 처리할 수 있다.

예를 들어,1 AWS S3 버킷에 새로운 원본 파일이 업로드되면, Argo Events는 이를 감지하고, Argo Workflow 형식으로 기술된 워크플로우를 시작한다. 워크플로우는1 파일을 추출하고, 필요한 변환을 수행한 다음, 결과물을 저장소에 저장한다. 이러한 방식으로 Argo Events와 Workflows 를 이용하면 데이터 정리 및 분석 과정을 효율화할 수 있다.

이 글에서는 예제 중심으로 위와 같은 과정을 설명하겠다. 먼저 필요한 개념 및 준비 과정부터 시작하겠다. 
...
- 파케이 (Parquet) 파일로 저장 (아티팩트 압축 사용 않음)
...
지금까지는 하나의 원본 CSV 에 대해서 진행하였다. 만약 동시에 여러 CSV 파일이 올라가면 어떻게 될까? 각 파일을 Argo Events 가 감지하고, 각각에 대해서 워크플로우가 실행되어야 할 것이다. 

날짜별 폴더에 그날의 테스트 점수가 여러 파일로 나누어 올라오는 경우를 생각해보자. 테스트를 위해 로컬에 `20230414` 폴더를 만들고 거기에 다음과 같은 내용의 파일들을 저장한다.
...
만약 각 파일의 ETL 을 수행한 뒤 집계, 예를 들어 최고 점수자를 찾으려면 어떻게 하면 될까?

다양한 방법으로 구현이 가능하겠으나 필자는 다음과 같이 접근해보겠다:
- 각 원본 파일의 ETL 결과를 MinIO `etlproj` 버킷의 결과 폴더 `output` 아래 날짜 폴더 `20230414` 로 올림
- Argo Events 가 결과 폴더 `output` 을 모니터링하다,
- 새 파일이 생성되면 *적당한 지연 시간* (여기서는 20초) 후에 집계 워크플로우 실행 
- 집계 워크플로우에서는
  - 진행 중인 기존 집계 워크플로우가 있으면 삭제 (중복 작업 방지)
  - 생성된 파일의 날짜 폴더 내 모든 파일을 내려 받고,
  - 집계 수행 후 완료되면 동일 폴더에 저장 

원본 파일이 불규칙 적으로 올라오는 경우 **지연 시간을 길게 둘 수록 중복 작업이 발생할 확률이 줄어들겠으나, 실시간성은 떨어질 것** 으로 트레이드 오프를 해야한다. 
...
MinIO UI 에서 `Upload File` 대신 `Upload Folder` 를 클릭하고, 만들어둔 `20230414` 폴더를 지정하여 업로드 하자.

문제가 없다면, 3 개 파일에 대해서 각각 동시에 워크플로우가 기동하고, `etlproj/output` 아래 결과 파일을 확인할 수 있을 것이다.

![3 Parquets](/assets/2023-04-14-17-16-36.png)

이제 결과 폴더를 모니터링하기 위해 새로운 이벤트 소스를 만들어야 한다. 아래 내용을 `agg-evtsrc.yaml` 파일로 저장한다.
...
`input` 이 아닌 `output` 폴더로, 확장자는 `.csv` 가 아닌 `.parquet` 를 대상으로 하고 있다.
...
이제 결과 취합을 위한 워크플로우가 필요한데, 스텝스 형식으로 먼저 이전 집계 워크플로우가 있으면 그것을 제거한 후, 다음 스텝으로 새 집게를 시작한다. 이를 위해 Argo CLI 가 설치된 컨테이너 이미지가 필요하다. 
...
이제 아래 내용을 `agg-workflow.yaml` 로 저장하자.
...
위 파일에서 호출 템플릿 `check-wait-agg` 을 통해 먼저 `check-prev` 템플릿이 호출되는데, 이것은 자신보다 앞서 진행중인 집계 워크플로우를 제거하는 역할이다. 이후 `wait` 템플릿에서 지연 시간을 주는데, 앞서 설명한 것 처럼 이것은 중복 작업을 최소화 하기 위함이다. 마지막으로 `agg` 템플릿에서 실제 집계 및 결과 업로드가 이루어진다.
...
집계 파일인 `agg.txt` 의 내용은 아래와 같을 것이다.
