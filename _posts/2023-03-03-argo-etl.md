...
스크립트 템플릿의 결과는 `stdout` 으로 출력되고, 그것은 `outputs.result` 로 얻을 수 있다. {% raw %}다른 템플릿에서 `{{tasks.<NAME>.outputs.result}}` 또는 `{{steps.<NAME>.outputs.result}}` 형식으로 참조할 수 있다.{% endraw %}
...
spec:
  ...
  - name: whalesay1
    container:
      ...
      command: [cowsay]
      # 글로벌 매개변수를 직접 참조 
      args: ["{% raw %}{{workflow.parameters.message}}{% endraw %}"]
```
  ...
  - name: output-parameter
    ...
    - - name: consume-parameter
        ...
        arguments:
          parameters:
          - name: message
            value: "{% raw %}{{steps.generate-parameter.outputs.parameters.hello-param}}{% endraw %}"
```
...
spec:
  ...
  - name: echo
    ...
    container:
      image: alpine:3.7
      command: [echo, "{% raw %}{{inputs.parameters.message}}{% endraw %}"]
...
`withItems` 대신에 아래와 같이 `withSequence` 로 교체하면 인자 변경없이 반복 수행도 가능하다 (이 경우 `{% raw %}{{item}}{% endraw %}` 형식으로 카운트를 참조할 수 있다).
  ...
  - name: loops-dag
    dag:
      ...
      - name: B
        ...
        arguments:
          parameters:
          - {name: message, value: "{% raw %}{{item}}{% endraw %}"}
        withItems:
        - foo
        - bar
        - baz
  ...
  - name: artifact-example
    ...
    - - name: consume-artifact
        ...
        arguments:
          ...
          - name: message
            from: "{% raw %}{{steps.generate-artifact.outputs.artifacts.hello-art}}{% endraw %}"
```
...
spec:
  ...
  - name: etl
    ...
    container:
      ...
      command: [python]
      args: ['/code/etl.py', {% raw %}'{{inputs.parameters.bucket}}', '{{inputs.parameters.key}}'{% endraw %}]
...
여기서는 지금까지 설명하지 않았던 세부적인 내용과, 까다로운 문제들을 살펴보겠다.
...
이제 MinIO 가 아닌 AWS S3 를 이용하는 경우를 살펴 보겠다. 원본 파일이 S3 에 올라오면 그 이벤트를 감지하여 ETL 워크플로우를 호출하는 식이다.
...
> 만약 자신의 PC 가 외부에서 접근 가능한 환경이라면, 별도의 EC2 인스턴스 생성없이 지금까지 사용하던 환경 그대로 진행해도 될 것이다.
...
![Event Type](/assets/2023-03-17-17-18-24.png)
...
spec:
  ...
  - name: etl
    ...
    container:
      image: haje01/argo-etl:0.1.1
      command: [python]
      args: ['/code/etl.py', {% raw %}'{{inputs.parameters.snsmsg}}'{% endraw %}]
      # 대상 S3 버킷에 읽기/쓰기 가능한 IAM 사용자 AccessKey / SecretKey 를 Secret 에서 환경변수로 전달 
      env: 
      - name: S3_ACCESS_KEY
        valueFrom:     # 환경 변수의 값 소스
          secretKeyRef:
