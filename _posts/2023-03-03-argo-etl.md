...
지금까지는 하나의 원본 CSV 에 대해서 진행하였다. 만약 동시에 여러 CSV 파일이 올라가면 어떻게 될까? 각 파일을 Argo Events 가 감지하고, 각각에 대해서 워크플로우가 실행되어야 할 것이다. 테스트를 위해 로컬에 `data` 폴더를 만들고 거기에 다음과 같은 내용의 파일들을 저장한다.
...
MinIO UI 에서 `Upload File` 대신 `Upload Folder` 를 클릭하고, 만들어둔 `data` 폴더를 지정하여 업로드 하자.

지금까지 작업에 문제가 없었다면, 3 개 파일에 대해서도 각각 워크플로우가 기동하는 것을 확인할 수 있을 것이다.

![3 Workflows](/assets/2023-04-13-16-20-57.png)
...
> 만약, Sensor 를 만든 뒤 ETL 코드를 수정하였다면 이를 반영하기 위해서는 코드 ConfigMap 재생성 및 그것을 사용하는 Sensor 까지 재생성해주어야 한다. 이런 불편함 때문에 다음 글에서 설명하는 Skaffold 같은 연속적 개발 툴을 이용하게 된다.
