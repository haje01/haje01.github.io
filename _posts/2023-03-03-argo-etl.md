...
MinIO UI 접속시 암호는 `admindjemals` 을 이용하면 된다.
...
Argo Events 에서 EventBus 는 이벤트 소스를 센서로 전달해주는 쿠버네티스 커스텀 리소스이다. 동작을 위해 EventBus 는 네임스페이스 내에 만들어져야 한다. 
...
하나 이상의 템플릿 호출 스텝으로 구성. 각 스텝은 정의된 순서대로 실행된다. 각 스텝은 호출의 리스트로 구성되는데, 리스트내 호출은 병렬로 실행된다.
...
매개변수는 워크플로우 및 템플릿에 대해 정의할 수 있는데, 워크플로우에서 정의된 매개변수를 글로벌 매개변수 (Global Paramter), 템플릿에서 정의된 매개변수를 로컬 매개변수 (Local Parameter) 라고 부른다. 
...
아래 예제에서는 `spec/arguments` 필드 아래에 글로벌 매개변수의 인자를 지정하고, 템플릿 명령 (`command`) 의 인자가 그것을 직접 참조하고 있다. 내용을 `global-parameters.yaml` 로 저장하자.
...
`argo submit` 의 옵션으로 지정된 인자 `goodbye world` 로 덮어 써져 출력되는 것을 로그에서 확인할 수 있다.
  ...
  - name: loop-example
    ...
    - - name: print-message
        ...
        arguments:
          ...
          - name: message
            value: "{{item}}"
        withItems:      alesay 템플릿을 아래의 두 인자값으로 동시에 각각 호출한다.
        - hello world           # item 1
...
이 예제는 `withItem` 대신 `withParam` 을 사용하고 있는데, 이렇게 하면 개별 항목을 템플릿에 하드코딩하지 않고 그것이 들어있는 매개변수를 지정할 수 있어 편리하다. 예제는 흥미롭게도 건네진 이미지로 컨테이너를 각각 실행하고 있다. 다음처럼 실행하면,
...
> 여기서 설명하는 내용은 향후 사용자의 독자적인 컨테이너 이미지가 필요한 경우를 위해서이다. 이 글의 예제를 따라하기 위해서라면 이미지를 별도로 만들 필요없이, 필자가 미리 만들어둔 이미지 `haje01/argo-etl:0.1.1` 를 이용하면 된다. 
...
AWS UI (Management Console) 에서 EC2 인스턴스를 만들자. `argo-test` 라는 이름으로 유형은 `t3.large`, 스토리지는 20GB 정도로 하자. 생성되었으면 다음과 같은 포트들을 보안 그룹에서 오픈한다:

- `8046` - Argo Workflows UI (소스: 내 IP)
- `9001` - Minio UI (소스: 내 IP)
- `12000` - Argo Events Webhook (소스: Anywhere IPv4)

그 다음 Public IP 를 확보 후 SSH 로 접속하여 [Docker](https://docs.docker.com/engine/install/ubuntu/) 와 [minikube](https://minikube.sigs.k8s.io/docs/start/), [Helm](https://helm.sh/docs/intro/install/) 설치 후, 처음처럼 Argo Events, Workflows, MinIO 도 설치한다.
...
먼저 이벤트를 모니터링할 S3 버킷의 ARN (Amazon Resource Names) 을 복사해두고, 다음으로 AWS UI 의 SNS 페이지로 가서 `argo-test` 이름으로 `표준` 유형 주제를 만든다 (최종 이름은 `argo-test.fifo` 가 됨).

이때 S3 버킷이 이 SNS 주제에 발행 (Publish) 할 수 있도록 권한 설정이 필요하다. `액세스 정책` 의 JSON 편집기에서 `Condition` 아래에 다음처럼 `ArnLike` 필드를 추가하고, 원래있던 `SourceOwner` 를 `SourceAccount` 로 변경한다.
...
해당 사용자 페이지로 이동 후 `권한 추가` 의 `인라인 정책 추가` 를 클릭하여 아래와 같은 인라인 정책을 추가하고 `argo-test` 라고 이름 짓자.
...
`정책 생성` 클릭 후 해당 사용자의 `보안자격 증명` 탭에서 `액세스 키 만들기`, `로컬 코드` 를 선택해 `Access Key` 와 `Secret Key` 를 복사해 둔다. 
...
그것으로 `sns-credential` 이라는 이름의 `Secret` 리소스 을 만든다.
...
ETL 코드를 위해서는 대상 S3 버킷의 `input/` 아래 파일들을 읽고, `output/` 아래에 결과 파일을 쓸 수 있는 권한이 필요하다. 이런 AWS UI 에서 이러 IAM 계정을 하나 만들고 그것의 AccessKey / SecretKey 에서 `s3-credential` 이라는 이름의 Secret 으로 만들자.
...
여기에서는 원본 데이터가 FTP 에 올라오는 경우를 예제로 커스텀 이벤트소스를 만드는 방법을 알아보겠다. Argo Events 는 FTP 이벤트소스를 지원하지 않는다. 그렇지만 앞에서 언급한 [제네릭 이벤트소스](https://argoproj.github.io/argo-events/eventsources/generic/) 와 커스텀 이벤트 서버를 이용해 FTP 를 위한 이벤트소스를 구현할 수 있다.
...
이후 서버가 FTP 접속을 위해 `ftp-secret` 이라는 이름의 `Secret` 리소스 을 만든다.
...
이제 이벤트소스를 위한 서버가 필요한데 (이것을 *이벤트 서버* 로 부르겠다), 그것은 다음과 같은 *gRPC 계약 (Contract)* 을 충족해야 한다. `ftp-code/` 디렉토리를 만들고 그쪽으로 이동 후, 아래 내용을 `generic.proto` 파일로 저장하자.
...
이벤트 서버 코드는 아래와 같다. `ftp-essrv.py` 로 저장하자.
...
이제 `ftp-code/` 폴더내 파일은 아래와 같을 것이다.
...
이제 상위 폴더로 이동 후, 다음처럼 `ftp-code/` 폴더에서 `ftp-code` ConfigMap 을 생성하고,

```
kubectl create configmap ftp-code --from-file=ftp-code/
```

FTP 서버 배포를 위해 다음과 같은 내용을 `ftp-essrv.yaml` 로 저장하고 적용한다.
...
metadata:
  name: ftp-essrv
  labels:
    app.kubernetes.io/name: ftp-essrv
spec:
  selector:
    matchLabels:
      app.kubernetes.io/name: ftp-essrv
...
`ftp.config` 필드에 기술하는 설정값 중 `passive` 는 FTP 의 Passive 모드 옵션인데, FTP 서버가 컨테이너에 있는 경우 필요하다.
...
아래는 FTP 에서 파일을 내려 받아 ETL 하는 코드이다. `etl-ftp.py` 로 저장하자.
...
# FTP 파일 정보
sinfo = sys.argv[2]
info = yaml.safe_load(sinfo)
path = info.get('path')
sz = info.get('sz')
dt = info.get('dt')

in_file = _get_candidate_names()
out_file = '/tmp/output.parquet'
cols = ['no', 'name', 'score']
...
ftp.login(user=FTP_USER, passwd=FTP_PASSWD)
print(f"Download {event} file '{path}'")
...
이벤트 발생시 위 코드를 실행할 워크플로우도 필요하다. 아래 내용을 `ftp-workflow.yaml` 로 저장하고 적용한다.
...
spec:
  ...
  - name: etl
    ...
    container:
      ...
      command: [python]
      args: ['/code/etl-ftp.py', '{{inputs.parameters.config}}']
...
센서는 앞서 만든 FTP 이벤트 소스와 워크플로우를 위해 다음과 같은 내용을 `ftp-sensor.yaml` 로 저장하고 적용한다.
  ...
  - template:
      ...
      k8s:
        ...
        - src:
            ...
            dataKey: notification.0.ftp.config
          dest: spec.arguments.parameters.0.value
```
...
지금까지 진행에서 몇 가지 문제가 있었다.
- 소스 코드를 ConfigMap 을 통해서 사용하는 것 
  - 변경이 있을 때마다 ConfigMap 을 업데이트 해야함
  - ConfigMap 을 통해 소스 코드가 쿠버네티스 클러스트 내부에서 유출 가능 
  - ConfigMap 은 etcd 를 이용해고 있어 최대 크기에 제한이 있음
- Dockerfile 및 코드가 변할 때마다 수동으로 빌드하고 관련 리소스를 재생성
  - 간단한 예제 정도의 개발에는 문제가 없겠으나, 조금 더 규모가 있는 프로젝트의 경우 수작업이 반복되어 장기적으로는 개발 효율이 떨어질 것 

이런 부분을 보완하기 위해서 쿠버네티스 환경에서 개발을 할 때는 개발용 워크플로우 툴을 사용하는데, 대표적인 것이 [Skaffold](https://skaffold.dev/) 이다. 다음과 같은 기능을 지원한다:

- 프로파일을 지원해서 프로파일별 초기 설치작업을 자동화
- Dockerfile 에 변경이 있을 때 자동으로 빌드해주고, 그것을 사용하는 리소스를 재생성 
- 소스 코드가 변경되면 자동으로 그것을 이용하는 컨테이너에 복사

Skaffold 의 이런 기능을 이용해서 개발을 하면서 실시간으로 리소스가 자동으로 갱신되는 것을 **지속 개발 (Continuous Developing)** 모드라 하는데, 처음부터 쿠버네티스 환경에서 개발을 시작하는 경우 없어서는 안될만큼 편리하다.
...
Skaffold 에 관한 모든 내용을 설명하는 것은 이 글의 범위를 넘어서는 것이기에, 자세한 것은 공식 페이지와 [이 글](https://blog.slashuniverse.com/25) 을 참고하도록 하자. 

필자는 지금까지 설명한 내용을 Skaffold 로 패키징하여 정리해두었는데, [이 주소]() 에서 그 내용을 확인할 수 있다.

실제 업무에서는 가급적 이러한 방식으로 개발 및 서비스가 진행되는 것이 바람직 할 것이니 꼭 참고하기 바란다.
...
- 기간 재작업이 필요한 경우 `withSequence` 로 Backkfill 수행
...
- 스크립트 템플릿 : 공유되지 않는 간단한 코드 
- 컨테이너 템플릿 + ConfigMap : 공유될 수 있고 너무 크지 않은 코드 (ConfigMap 은 1MB 크기 제한)
- 컨테이너 템플릿 + 코드를 포함한 이미지 : 실제 서비스용 코드 

### 끝으로

이 문서에는 Argo Events / Workflows 기능의 일부만을 설명하고 있다. 공식 문서 및 예제를 참고하여 더 자세한 내용을 확인하도록 하자.
