...
> - `outputs` ([문서](https://argoproj.github.io/argo-workflows/)
>   - `artifacts`, `exitCode`, `parameters`, `result` fields/#outputs))
...
spec:
  ...
  - name: loop-map-example
    ...
    - - name: test-linux
        ...
        arguments:
          parameters:
          - name: image
            value: "{% raw %}{{item.image}}{% endraw %}"
          - name: tag
            value: "{% raw %}{{item.tag}}{% endraw %}"
        withItems:
        # 컨테이너 이미지 이름과 태그 정보
  ...
  - name: cat-os-release
    ...
    container:
      ...
      image: "{% raw %}{{inputs.parameters.image}}:{{inputs.parameters.tag}}{% endraw %}"
      command: [cat]
      args: [/etc/os-release]
```
