...
* *이탈 확인 일수 (churn_test_days, c_d)* : 각 휴식 유저에 대해, 이탈 확인에 필요한 날자 수. 휴식 판정일 후 이 일수까지의 데이터에 대해 이탈 확인을 수행한다.
  ...
  * 즉, 이 유저의 중단일부터 7일 후까지 단위 기간 한계 접속 일수 (k^mar) 이상의 접속이 없었으면 이탈 (churn) 로, 아니면 휴식 (idle) 으로 판정한다.

$$ c_d = \lceil f \times i_r \times c_r \rceil = \lceil i_d \times c_r \rceil$$

* *이탈 확인 기간 (churn_test_term)* : 휴식 기간 이후 이탈 확인 일수 동안의 기간.

* *최대 이탈 확인 일수 (max_churn_test_days, c_d^max)* : 모든 중단자의 이탈 확인 일수 중 가장 큰 값. 최대 접속 주기 (f^max), 휴식 확인률 (i_r) 및 이탈 확인률 (c_r) 을 곱해 구한다.
...
예측 후 실제 모델의 성능 평가를 위해 검증이 꼭 필요하다.

#### 용어와 개념

* *한계 검증 데이터 일수 (verify_data_margin_days, V_d^mar)* : 모델의 성능 평가를 위해 예측 결과 후 검증에 필요한 일수.
  * 최대 휴식 확인 일수 (i_d^max), 그리고 최대 이탈 확인 일수 (c_d^max) 의 합이다.
  * 예로, 최대 휴식 확인 일수 i_d^max = 7 이고 최대 이탈 확인 일수 c_d^max = 7 이면 한계 학습 데이터 일수 L_d^mar 은 14 가 된다.

$$V_d^{mar} = i_d^{max} + c_d^{max}$$
...
예측일 이후 최대 휴식 확인 일수 (i_d^max) + 최대 이탈 확인 일수 (c_d^max) 데이터 확보
예측 결과 내 모든 유저에 대해
  접속 주기 (f) 에서 휴식 확인 일수 (i_d) 를 구함
  휴식 확인 데이터의 단위 기간 접속 일수 (k) 가 단위 기간 한계 접속 일수 (k^mar) 보다 작으면
    휴식 판정
