...
* *중단* (pause) : 여행, 시험, 기타 개인적인 이유로 잠시 한동안 접속하지 않는 유저의 상태
...
* *유효 접속 일수 (effect_plays, e_p)* : 군집 판정, 학습이나 예측에서 몇 개의 접속일 데이터를 사용할지를 나타냄 (초모수)
  * 예로 e_p = 4 인 경우, 군집 판정에서는 판정 시작일부터 4 접속일의 데이터를, 이탈 예측에서는 예측일 이전 4 접속일의 데이터를 사용하겠다는 뜻이다.
...
* *이탈 의심률 (churn_absent_rate, r_c)* : 유저의 결석률이 이 값보다 크거나 같으면 이탈 의심을 한다. 오탐이나 미탐이 적도록 설정 (초모수)
  * 예로, r_c = 3 이면 세 번의 접속 주기만큼 결석하면 이탈으로 의심한다는 뜻

* *이탈 의심 일수 (churn_suspect_days, c_d)* : 유저의 이탈 의심에 필요한 날자 수. 이탈 의심률 (r_c) 과 접속 주기 (f) 에서 계산
  * 예로, 접속 주기 f = 1.75 인 유저에 대해 이탈 의심률 r_c = 3 으로 하면, 이탈 의심 일수 c_d = 6 로, 마지막 접속일 후 6 일이 지나야 이탈 의심이 가능하다.

$$c_d = \lceil f \times r_c \rceil $$

* *이탈 의심 기간 (churn_suspect_term)* : 유효 데이터 기간 이후 이탈 의심 일수 동안의 기간.

* *최대 이탈 의심 일수 (max_churn_suspect_days, c_d^max)* : 모든 이탈 의심 일수 중 가장 큰 값. 최대 접속 주기 (f^max) 와 이탈 의심률 (r_c) 로 계산
  * 예로 최대 접속 주기 f^max = 2.33 이고 이탈 의심률 r_c = 3 이면 c_d^max = 7 이다.

$$c_d^{max} = \lceil f^{max} \times r_c \rceil $$

* *이탈 확인률 (churn_test_rate, r_t)* : 각 이탈 의심 유저에 대해, 이탈 확인에 필요한 일수를 계산하기 위한 비율. 이탈 의심률에 대한 비율로, r_t = 1 이면 이탈 의심률과 같은 일수 경과 후 이탈 판정을 하게 된다. (초모수)
  * 너무 작으면 제대로된 검증을 할 수 없고, 너무 크면 검증에 시간이 오래 걸리며 지나치게 엄격한 검증이 될 수 있다.

* *이탈 확인 일수 (churn_test_days, t_d)* : 각 이탈 의심 유저에 대해, 이탈 의심일 후 이 일수까지의 날자 경과 후 이탈 확인을 수행한다. 이탈 의심시 접속 주기 (f), 이탈 의심률 (r_c) 및 이탈 확인률 (r_t)을 곱해 구한다.
  * 예로, 어떤 이탈 의심 유저의 접속 주기가 f = 2.33 이었고, 이탈 의심률 r_c = 3, 이탈 확인률 r_t = 1 이면, 이탈 확인 일수 t_d = 7 이다
  * 즉, 이 유저의 이탈 의심일부터 7일 후까지 단위 기간 한계 접속 일수 (k^mar) 이상의 접속이 없었으면 이탈 (churn) 로, 아니면 중단 (pause) 으로 판정한다.

$$ t_d = \lceil f \times r_c \times r_t \rceil = \lceil c_d \times r_t \rceil$$

* *이탈 확인 기간 (churn_test_term)* : 이탈 의심 기간 이후 이탈 확인 일수 동안의 기간.

* *최대 이탈 확인 일수 (max_churn_test_days, t_d^max)* : 모든 이탈 의심자의 이탈 확인 일수 중 가장 큰 값. 최대 접속 주기 (f^max), 이탈 의심률 (r_c) 및 이탈 확인률 (r_t) 을 곱해 구한다.
  * 이탈 확인을 위해 이 일수 만큼의 데이터를 확보해야 한다.

$$ t_d^{max} = \lceil f^{max} \times r_c \times r_t \rceil = \lceil c_d^{max} \times r_t \rceil$$


* *한계 학습 데이터 일수 (learn_data_margin_days, L_d^mar)* : 학습용 데이터는 군집 결정 및 이탈 확인이 가능해야 하는데, 이를 위해 필요한 최소한의 데이터 날자 수.
  * 최대 유효 데이터 일수 (e_d^max) 와 최대 이탈 의심 일수 (c_d^max), 그리고 최대 이탈 확인 일수 (t_d^max) 의 합이다.
  * 예로, 최대 유효 데이터 일수 e_d^max = 28 이고 최대 이탈 의심 일수 c_d^max = 14, 그리고 최대 이탈 확인 일수 t_d^max = 7 이면 한계 학습 데이터 일수 L_d^mar 은 49 가 된다.

$$L_d^{mar} = e_d^{max} + c_d^{max} + t_d^{max}$$
...
* *수집 윈도우 크기 (col_window_width, w_w)* : 수집 윈도우의 범위.
  * 수집 윈도우 크기 결정을 위해, 먼저 수집 윈도우 시작일부터 단위 기간 동안 유저의 접속 일수로 단위 기간 접속 일수 (k) 를 구한다.
  * k 에서, 유효 데이터 일수 (e_d), 이탈 의심 일수 (c_d) 및 이탈 확인 일수 (t_d) 를 구한다. 세 값의 합이 윈도우의 크기가 된다.
  * 유효 데이터 일수의 데이터는 학습에 사용되고, 이탈 의심 일수의 데이터는 라벨링에 사용된다.

$$w_w = e_d + c_d + t_d$$

* *수집 윈도우 종료일 (col_window_end, w_e)* : 수집 윈도우 시작일 부터 유효 데이터 일수 및 이탈 의심 일수로 계산

$$w_e = w_b + w_w$$

여기까지의 설명을 그림으로 나타내면 아래와 같다:

![수집 윈도우](/assets/2020-05-20-12-03-52.png)
...
* 중단 (pause) 으로 판정된 경우, 이탈 확인 기간내 첫 접속일부터 다음 윈도우가 시작하도록 수집 윈도우 스텝을 설정한다.
  * 이렇게 하는 이유는, 중단 시기의 결석일이 그대로 사용되면 윈도우의 크기가 지나치게 커지기 때문이다.
...
학습 데이터 내 모든 유저에 대해
  ...
  일의 데이터로 수집 윈도우 크기 (w_w) 구함
  수집 윈도우에 대해
    유효 접속 일수 (e_p) 에 미치지 못하거나, 수집 윈도우 종료일 (w_e) 이 학습 데이터 종료일보다 크면
      다음 유저로
    윈도우내 유효 일수 데이터의 단위 기간 접속 일수 k 가 단위 기간 한계 접속 일수 (k^mar) 미만이면
      다음 유저로

    (필요시 k 로 군집 결정)
    윈도우내 유효 데이터 일수 (e_d) 의 피처 데이터 (X) 를 군집의 학습 데이터에 추가
    윈도우내 이탈 의심 일수 (c_d) 및 이탈 확인 일수 (t_d) 에서 유저의 상태를 결정하고 라벨 (y) 을 학습 데이터에 추가
    이탈로 판정되었으면
      다음 유저로
    중단으로 판정되었으면
      이탈 확인 기간내 첫 접속일에서 다음 윈도우가 시작되도록 윈도우 스텝 산정
...
![수집 과정](/assets/2020-05-20-12-02-56.png)
...
* 이탈 의심률 (r_c)
  * 학습 데이터에서 이탈 확인 일수 (t_d) 및 이탈 확인률 (r_t) 로 판정 오류를 조사하고,
...
* 예측 데이터 내 결석률이 이탈 의심률 이상인 유저는 기이탈로 판단해 예측 대상에서 제외한다.
