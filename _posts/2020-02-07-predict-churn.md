...
* *중단* (stop) : 서비스 이용의지는 있으나 불가피한 여행, 시험, 기타 개인적인 이유로 한동안 접속하지 않는 유저의 상태
...
* *그룹 (group, g)* : 접속 빈도에 따라 학습 데이터와 예측 모델 구분해 사용하면 더 좋은 성능을 기대할 수 있는데, 이글에서는 이를 위한 분류를 룹이라 한다. 그룹은 유저의 단위 기간 접속 일수 (k) 를 이용해, 서비스별 휴리스틱으로 정의한다. 예로, 앞에서 다음과 같이 그룹을 설정하였다.
  * g^a - 단위 기간 접속일이 6, 7 일인 유저군
  * g^b - 단위 기간 접속일이 3, 4, 5 일인 유저군
  * 이때, 단위 기간 접속일이 6 일인 유저는 그룹 g^a, 3 일인 유저는 그룹 g^b 에 속하게 된다.

* *그룹 최대 접속 주기 (group_max_play_freq, f_g^max)* : 특정 그룹내 유저의 최대 접속 주기.
  * 예로 단위 기간 접속일이 6, 7 인 유저가 그룹 g^a 에 속한다면, 이 그룹의 최대 접속 주기는 7/5 = 1.66 이다.
...
* *유효 데이터 일수 (group_data_days, e_d)* : 그룹별로 유효 접속 일수에 해당하는 데이터 수집에 필요한 기간 일수.
  * 그룹 최대 접속 주기 (f_g^max) 와 유효 접속 일수 (e_p) 를 곱해 산정한다.
  * 예로 단위 기간 접속일이 6, 7 인 g^a 그룹에서 유효 접속 일수 e_p = 4 인 경우, 그룹 최대 접속 주기 f_g^max=1.66 이기에 유효 데이터 일수 e_d = 7 이다. 즉 7 일치 데이터가 있어야 한다.
  * 단위 기간 접속일이 3, 4, 5 인 g^b 그룹에서는 e_d = 10 이다.

$$e_d  = \lceil f_g^{max} \times e_p \rceil$$
...
학습 데이터는 이후 설명할 수집 윈도우를 통해 유효 데이터, 중단 확인 데이터, 이탈 확인 데이터의 세 부분으로 나누어 수집한다. 먼저 중단 (stop) 관련 용어를 살펴보자.

* *중단 확인 데이터 (stop_test_data)* : 앞에서 설명한 유효 데이터 일수 (e_d) 이후 아래에서 설명할 중단 확인 일수 (s_d) 만큼의 데이터로, 중단 여부 판단에 사용된다.

* *중단 확인률 (stop_test_rate, s_r)* : 각 유저에 대해 중단 확인에 필요한 일수를 계산하기 위한 값. 접속 주기 (f) 에 대한 비율로, 오탐이나 미탐이 적도록 설정 (초모수)

* *중단 확인 일수 (stop_test_days, s_d)* : 유저의 중단 확인에 필요한 날자 수.
  * 유효 데이터에서 구한 접속 주기 (f) 에 중단 확인률 (s_r) 곱해 구한다.
  * 예로, 접속 주기 f = 1.75 인 유저에 대해 중단 확인률 s_r = 3 으로 하면 중단 확인 일수 s_d = 6 로, 유효 데이터 이후 6 일의 데이터가 있어야 중단 판단이 가능하다.
  * 이 기간내 접속일 수가 한계 접속 일수 (k^mar) 미만이면 중단 (stop) 으로 판정한다.

$$s_d = \lceil f \times s_r \rceil $$

* *최대 중단 확인 일수 (max_stop_test_days, s_d^max)* : 모든 중단 확인 일수 중 가장 큰 값. 최대 접속 주기 (f^max) 와 중단 확인률 (s_r) 로 계산
  * 예로 최대 접속 주기 f^max = 2.33 이고 중단 확인률 s_r = 3 이면 s_d^max = 7 이다.

$$s_d^{max} = \lceil f^{max} \times s_r \rceil $$
...
* *이탈 확인률 (churn_test_rate, c_r)* : 각 중단 유저에 대해, 이탈 확인에 필요한 일수를 계산하기 위한 값. 중단 확인 일수 (s_d) 에 대한 비율로, 오탐이나 미탐이 적도록 설정 (초모수)
  * c_r = 1 이면 중단 확인 일수와 같은 날자 경과 후 이탈 판정을 하게 된다.
  * 너무 작으면 제대로된 검증을 할 수 없고, 너무 크면 검증에 시간이 오래 걸리며 지나치게 엄격한 검증이 될 수 있다.

* *이탈 확인 일수 (churn_test_days, c_d)* : 각 중단 유저에 대해, 이탈 확인에 필요한 날자 수. 중단 판정일 후 이 일수까지의 데이터에 대해 이탈 확인을 수행한다.
  * 중단 확인 일수 (s_d) 에 이탈 확인률 (c_r) 을 곱해 구한다.
  * 이 기간내 접속일 수가 한계 접속 일수 (k^mar) 미만이면 이탈 (churn)로 판정하고, 아니면 중단 (stop) 을 유지한다.
  * 예로, 어떤 중단 유저의 접속 주기가 f = 2.33 이었고, 중단 확인률 s_r = 3, 이탈 확인률 c_r = 1 이면, 이탈 확인 일수 c_d = 7 이다
  * 즉, 이 유저의 중단일부터 7일 후까지 단위 기간 한계 접속 일수 (k^mar) 이상의 접속이 없었으면 이탈 (churn) 로, 아니면 중단 (stop) 으로 판정한다.

$$ c_d = \lceil f \times s_r \times c_r \rceil = \lceil s_d \times c_r \rceil$$

* *이탈 확인 기간 (churn_test_term)* : 중단 기간 이후 이탈 확인 일수 동안의 기간.

* *최대 이탈 확인 일수 (max_churn_test_days, c_d^max)* : 모든 중단자의 이탈 확인 일수 중 가장 큰 값. 최대 접속 주기 (f^max), 중단 확인률 (s_r) 및 이탈 확인률 (c_r) 을 곱해 구한다.
  * 이탈 확인을 위해 이 일수 만큼의 데이터를 확보해야 한다.

$$ c_d^{max} = \lceil f^{max} \times s_r \times c_r \rceil $$

> 유효 데이터 일수 (e_d) 는 그룹별로 결정하고, 중단 확인 일수 (s_d) 및 이탈 확인 일수 (c_d) 는 유저별로 결정하는 것에 주의하자.

* *한계 학습 데이터 일수 (learn_data_margin_days, L_d^mar)* : 학습용 데이터는 그룹 결정 및 이탈 확인이 가능해야 하는데, 이를 위해 필요한 최소한의 데이터 날자 수.
  * 최대 유효 데이터 일수 (e_d^max) 와 최대 중단 확인 일수 (s_d^max), 그리고 최대 이탈 확인 일수 (c_d^max) 의 합이다.
  * 예로, 최대 유효 데이터 일수 e_d^max = 10 이고 최대 중단 확인 일수 s_d^max = 7, 그리고 최대 이탈 확인 일수 c_d^max = 7 이면 한계 학습 데이터 일수 L_d^mar 은 24 가 된다.

$$L_d^{mar} = e_d^{max} + s_d^{max} + c_d^{max}$$

> 터 일수는 어디까지나 최소한의 값이다. 이에 따라 데이터를 확보해도 그 기간동안 유저 행동에 변동이 있으면 데이터가 부족해질 수 있기에, 실제 학습 데이터 일수는 한계 학습 데이터 일수 보다 여유있게 잡자.
...
* *수집 윈도우 크기 (col_window_width, w_w)* : 수집 윈도우의 범위.
  ...
  * 여기서 중단 확인 일수 (s_d) 및 이탈 확인 일수 (c_d) 를 구한다.
  * 유효 데이터 일수 (e_d), 중단 확인 일수 (s_d) 및 이탈 확인 일수 (c_d) 의 합이 윈도우의 크기가 된다.
...
![수집 윈도우](/assets/2020-05-26-17-58-06.png)
...
* 수집 윈도우 데이터에서 유저의 상태가 중단 (stop) 으로 판정된 경우, 이탈 확인 기간내 첫 접속일부터 다음 윈도우가 시작하도록 수집 윈도우 스텝을 설정한다.
...
학습 데이터 내 모든 유저에 대해
  ...
  수집 윈도우에 대해
    데이터가 최대 유효 데이터 일수 (e_d^max) 미만이면
      다음 유저로
    최대 유효 데이터 일수 (e_d^max) 동안의 데이터로 유저의 단위 기간 접속 일수 (k) 와 접속 주기 (f) 구함
    단위 기간 접속 일수 (k) 가 단위 기간 한계 접속 일수 (k^mar) 미만이면
      다음 유저로

    단위 기간 접속 일수 (k) 에서 그룹 결정
    그룹의 유효 데이터 일수 (e_d) 동안의 데이터에서 접속 주기 (f) 와 중단 확인 일수 (s_d) 구함
    중단 확인 데이터가 부족하면
      다음 유저로

    중단 확인 데이터의 단위 기간 접속 일수 (k) 가 단위 기간 한계 접속 일수 (k^mar) 보다 작으면
      중단 판정
...
* 두 번째 스텝 (단위 기간 접속 일수 k = 4 인 경우)
  ...
  * 중단 확인 일수 s_d = 8
  * 이탈 확인 일수 c_d = 9
  * 윈도우 크기 w_w = e_d + s_d = 28

![수집 과정](/assets/2020-05-26-18-12-19.png)
