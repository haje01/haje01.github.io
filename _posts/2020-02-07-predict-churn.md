...
* *정지* (pause) : 여행, 시험, 기타 개인적인 이유로 잠시 한동안 접속하지 않는 유저의 상태
...
* *이탈 확인 일수 (churn_test_days, t_d)* : 각 이탈 의심 유저에 대해, 이탈 의심일 후 이 일수까지의 날자 경과 후 이탈 확인을 수행한다. 이탈 의심시 접속 주기 (f), 이탈 의심률 (r_c) 및 이탈 확인률 (r_t)을 곱해 구한다.
  ...
  * 즉, 이 유저의 이탈 의심일부터 7일 후까지 단위 기간 한계 접속 일수 (k^mar) 이상의 접속이 없었으면 이탈 (churn) 로, 아니면 정지 (pause) 로 판정한다.
...
학습 데이터 내 모든 유저에 대해
  ...
  수집 윈도우에 대해
    ...
    윈도우내 이탈 의심 일수 (c_d) 의 결석일로 이탈 여부 (y) 를 군집의 학습 데이터에 추가
    이탈했으면
      다음 유저로
