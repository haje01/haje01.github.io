...
학습 데이터 내 모든 유저에 대해
  ...
  수집 윈도우에 대해
    ...
    단위 기간 접속 일수 (k) 가 단위 기간 한계 접속 일수 (k^mar) 미만이면
      다음 유저로
    마지막 접속일 (l) 에서 결석일 (a) 을 구함 (피처로 사용)
...
지금까지 설명한 개념과 과정의 이해를 돕기 위해, 추가적인 예를 들어 설명하겠다. 아래는 단위 기간 접속 일수가 k = 6 에서 k = 4 으로 변하고 이탈한 유저의 예이다.
* 공통 변수
  * 단위 기간 U = 7
  * 휴식 확인률 i_r = 5
  * 이탈 확인률 c_r = 1
...
* 두 번째 스텝 (단위 기간 접속 일수 k = 4 인 경우)
  ...
  * 휴식 확인 일수 i_d = 8
  * 이탈 확인 일수 c_d = 9
  * 윈도우 크기 w_w = e_d + i_d = 28

![수집 과정](/assets/2020-05-27-10-57-38.png)
