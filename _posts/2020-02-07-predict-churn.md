...
* *이탈 검증 일수 (churn_test_days, t_d)* : 검증 대상일에서 이 일수 전부터의 이탈자에 대해 검증을 수행.
  * 최대 접속 주기 (f^max) 와 이탈 검증률을 곱해 정한다.
  * 예로, 최대 접속 주기 f^max = 2.33 이고 이탈 검증률 r_t = 6 이면, 이탈 검증 일수 t_d = 14 이다
