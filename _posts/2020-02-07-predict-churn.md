...
* 군집 분류:
  ...
  * g^3 - 주당 접속일이 3 일인 유저군
  * g^4 - 주당 접속일이 4 일인 유저군
  * g^5 - 주당 접속일이 5 일인 유저군
  * g^6 - 주당 접속일이 6 일인 유저군
...
* *마지막 접속일 (last_play, l)* : 유저의 기간내 마지막 접속일
* *기간 접속일 수 (plays, p_d)* : 기간 내 유저가 접속한 일 수
  * 10 일 동안 4번 접속한 경우 $p_{10} = 4$
* *주당 접속일 수 (week_plays, w)* : 기간 내 유저의 접속일 수를 주 단위로 계산한 것.
...
* *군집 (group, g^w)* : 기간 동안 유저가 속하는  집합. 주당 접속일 수 (w)로 결정한다.
  * 예로, g^3 군집은 주당 접속일이 3 일인 유저의 그룹이다.
* *최대 군집 수 (group_max, m)* : 다룰 군집의 수
* *군집 셋 (group set, G_m)* : 군집의 집합

$$ G_m = \{g^x \vert x \in \mathbb Z \land 1 \leq x \leq m \}$$

 * 최대 군집 수 m = 7 인 군집 셋은 다음과 같다:

$$ G_7 = \{ g^1, g^2, g^3, g^4, g^5, g^6, g^7 \}$$

* *군집 결정 접속일 수 (group_decide_plays, G_p)* : 군집 집합에서 유저의 군집 결정에 필요한 최소 접속일 수 (초모수)
  * 예로 G_p = 3 이라는 것은, 어떤 유저의 3번째 기간 접속일에 주당 접속일 (w) 을 계산해 군집을 배정할 수 있다는 것이다.
  * 이 값이 클 수록 정확한 군집 배정이 가능하나, 동시에 많은 데이터가 필요하다.
* *군집 결정 일 수 (group_decide_days, G_d)* : 유저의 군집을 결정하는데 필요한 데이터 일 수. 유저의 주당 접속일 수 (w) 와 군집 결정 접속일 수 (G_p) 으로 계산.
  * 예를 들어 G_p = 4 으로 w = 1 인 유저의 군집을 결정하려면 G_d = 28 로, 28 일치 데이터가 필요하다.

$$G_d = G_p \times \frac {7} {w} $$

* 위의 예에서 알 수 있듯, 주당 접속일 수가 작은 유저를 정확히 분류하려면 필요한 데이터의 기간 일 수 (d) 가 늘어나는데, 이는 최근 경향을 반영하지 못하는 *개념 표류 (Concept Drifting)* 및 데이터 부족을 초래할 수 있기에, 적절한 군집 결정 접속일 수 (G_d) 의 선정이 중요하다.

* *접속 예정일 (due_play_day, e)* : 기간 내 유저의 마지막 참석일 이후 예상되는 다음 접속일. 마지막 접속일 (l) 과 접속 주기 (f) 로 계산.

$$e = l + f$$

* *결석률 (absent_rate, r)* : 유저의 접속 예정일 (e) 이후, 예측일 (t) 까지 결석일 수를 군집 접속 주기 주기에 대한 비율로 나타낸 것 (0 이상)
  * 예로, 접속 주기 f = 3 일인 군집에 속한 유저가, 6일 동안 결석했다면 (= t - e), 결석율 r = 1.0
...
* *최대 이탈 판정일 수 (max_churn_decide_days, c_max)* : 모든 이탈 판정일 수 중 가장 큰 값
...
* *기간 학습 데이터 (learn_data, L_T 줄여서 L)* : 대상 기간 (T) 에 대한 학습 데이터. 줄여서 *학습 데이터* 로 부름
* *기간 학습 데이터 일 수 (learn_data_days, L_d)* : 기간 내 학습 데이터의 날자 수
...
앞에서 주간 접속일 수 (w) 로 유저의 군집을 분류하는 것을 살펴보았다. 학습 데이터에서 *군집 학습 데이터 셋 (group_learn_data, O)* 를 다음과 같이 준비한다:

* 군집 집합 (G) 내 각 군집에 대해 각각의 학습 데이터 o^1, o^2, ..., o^7 을 만들게 됨

$$O = \{ o^1, o^2, o^3, o^4, o^5, o^6, o^7 \}$$

* 같은 유저도 경과에 따라 접속 주기가 계속 달라 질 수 있기에, 이어 설명할 수집 윈도우를 이용해 해당 유저의 접속 주기를 지속적으로 분류하고, 군집에 배정한다.
* 예 1 : 학습 데이터 내 어떤 유저가 g^5 -> g^3 -> g^1 순으로 군집이 변하다 이탈
  * 이 유저의 g^5 시기 데이터를 o^5 에 배정하고 비이탈로 라벨링
  * 이 유저의 g^3 시기 데이터를 o^3 에 배정하고 비이탈로 라벨링
  * 이 유저의 g^1 시기 데이터를 o^1 에 배정하고 이탈로 라벨링
* 예 2 : 학습 데이터 내 어떤 유저가 g^2 군집에서 바로 이탈
  * 이 유저의 전체 데이터를 g^2 에 배정하고 이탈로 라벨링

데이터 양을 늘리기 위해 유저별로 일종의 슬라이딩 윈도우인 *수집 윈도우 (col_window, W)* 를 적용해 데이터를 수집한다.

* 수집 윈도우의 시작은 학습 데이터 내 유저의 최초 접속일 부터
* *수집 윈도우 크기 (col_window_width, W_w)* : 윈도우 크기는 군집 결정 및 이탈 판정이 가능하도록 설정
  * 즉, 군집 결정 일 수 (G_d) 와 최대 이탈 판정일 수 (c_max) 의 합보다 커야한다.
