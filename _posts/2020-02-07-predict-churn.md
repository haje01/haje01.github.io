...
* *접속 주기 (play_freq, f)* : 유저가 몇 일마다 접속하는지의 값. 기간 일 수 (d) 와 접속일 수 (p) 로 계산
  * 예로, 기간일 수 d = 7 동안 접속일 수 p = 3 인 경우 접속 주기 f = 1.75
 ...
 * 단위 기간 (U) 과 단위 기간 한계 접속일 수 (k^mar) 에 의해 군집 집합이 결정된다. 예로, 단위 기간 U = 7 이고 단위 기간 한계 접속일 수 k^mar = 3 인 경우 군집 집합은 다음과 같다:
...
* *최대 이탈 판정일 수 (max_churn_decide_days, c^max)* : 모든 이탈 판정일 수 중 가장 큰 값. 최대 접속 주기 (f^max) 와 이탈 결석률 (r_c) 로 계산
  * 예로 최대 접속 주기 f^max = 2.33 이고 이탈 결석률 r_c = 3 이면 c^max = 7 이다.
...
이탈 예측은 시계열 예측으로, 먼저 다음과 같은 유저별 피처를 일단위로 구성하고, 여기에서 시간 경과에 따른 경향을 피쳐화한다.
...
하나 또는 그 이상의 피처에서 합성된 피처로, 이탈 예측의 중요한 변수가 될 수도 있다. 이에 예전에 소개한 Featuretools 의 [심층 피처 합성](https://haje01.github.io/2019/12/27/deep-feature-synthesis.html) 방식을 이용하면 손쉽게 다양한 합성피처를 생성할 수 있다.

### 피처 필터링

수작업에 합성된 것까지 포함하면 굉장히 많은 피처를 다루게 될 것이다. 본격적인 모델 학습 전에 피처 필터링 단계를 통해 피처 수를 줄이는 것이 좋다.
