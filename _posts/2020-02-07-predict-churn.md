...
* 유저를 한 단위 기간당 플레이한 날의 평균에 따라 다음과 같이 분류한다. (이후 설명할 단위 기간이 U = 7 이고, 단위 기간당 최소 접속일 수 k^min = 1 인 경우)
* 군집 분류:
...
* 최대 접속 주기 이상, 즉 일주일에 한 번 미만으로 플레이하는 유저는 학습 및 예측 대상으로 하지 않는다.
...
* *기간 (term, T)* : 시작일 (begin, b) 부터 종료일 (end, e) 까지의 범위를 지칭. 종료일 (e) 는 제외한다.
* *단위 기간* (unit_term, U) : 주로 사용하는 기간의 크기. 여기서는 U = 7, 즉 일주일
...
* *기간 접속일 수 (plays, p_d 간단히 p)* : 기간 내 유저가 접속한 일 수
  * 10 일 동안 4번 접속한 경우 $p_{10} = 4$

* *단위 기간당 접속일 수 (unit_plays, k)* : 기간 내 유저의 접속일 수를 주 단위 기간으로 계산한 것.
  * 예로, 단위 기간이 일주일 U = 7 일때 유저가 d = 14 일 동안 p = 2 번 접속한 경우 k = 1
  * 단위 기간당 접속일로 유저가 속한 군집을 결정한다.
  * 최대값은 7

$$k = \lceil \frac {p} {d} \rceil \times U$$

* *단위 기간당 최소 접속일 수 (unit_min_play, k^min)* : 단위 기간당 접속일 수 (k) 의 최소값. 유저의 단위 기간당 접속일이 이 값 미만이면 제외한다. (초모수)

* *접속 주기 (play_freq, f)* : 유저가 몇 일마다 접속하는지의 값. 기간 일 수 (d) 와 그 동안의 접속일 수 (p) 로 계산

$$f = \lceil \frac {d} {p} \rceil = \lceil \frac {U} {k} \rceil $$

<!-- * *최대 접속 주기 (max_play_freq, f^max)* : 접속 주기의 최대값. 접속 주기가 이 값 이상인 유저는 제외한다. (초모수) -->

<!-- * *최대 단위 기간당 접속일 수 (max_unit_plays, k^max)* : 단위 기간당 접속일 중 가장 큰 값. 최소 접속 주기의 역

$$k^{max} = \frac {7} {min(f)}$$ -->

* *군집 (group, g^k)* : 기간 동안 유저가 속하는  집합. 단위 기간당 접속일 수 (k) 로 결정되며, 접속 주기와 역의 관계
  * 예로, g^3 군집은 단위 기간당 접속일이 3 일인 유저의 군집이다.

* *군집 집합 (group set, G_m)* : 전체 군집의 모임

$$ G = \{g^m \vert m \in \mathbb Z \land k^{min} \leq m \leq U \}$$

 * 단위 기간 (U) 와 단위 기간당 최소 접속일 수 (k^ 에 의해min_ 군집 집합이 결정된다. 예로, U = 7 이고 k^min = 3 인 경우 군집 집합은 다음과 같다:

$$ G = \{ g^3, g^4, g^5, g^6, g^7 \}$$

* *유효 접속일 수 (effect_plays, e_p)* : 기간 종료일 기준으로 몇 번째 최근 접속일 데이터까지를 사용할지를 나타냄 (초모수)
  * 예로 e_p = 4 이라는 것은, 유저의 기간내 최근 4 개 접속일의 데이터를 사용하겠다는 뜻이다.
...
* *최대 유효 데이터 일 수 (max_effect_data_days, e_d^{max})* : 모든 유저의 유효 접속일 수 중 가장 큰 값. 최대 접속 주기 (f^max) 와 유효 접속일 수 (e_p) 으로 계산.
  * 예를 들어 f^max = 7 과 e_p = 4 를 이용하면 e_d = 7 x 4 = 28 로, 28 일분 데이터의 검색이 필요하다.
  * 그러나, 그 기간동안 실제 유저의 접속일은 4일 정도일 것이기에, 그 정도의 데이터만 활용할 수 있다.

$$e^{max}_d = f^{max} \times {e_p}$$

* 위의 예에서 알 수 있듯, 실제 활용하는 데이터에 비해 다루어야할 데이터가 너무 많아지기에, 적절한 유효 접속일 수 (e_p) 및 최대 접속 주기 (f^max) 의 선정이 필요하다. -->
...
$$O = \{ o^m : m \in \mathbb Z, k^{min} \leq m \leq U\}$$

* *군집 유효 데이터 일 수 (group_effect_data_days, e_d^g)* : 군집 유저의 유효 데이터 일 수와 같다.

* *군집 학습 데이터 일 수 (group_learn_data_days, o_d^g)* : 군집별 학습 데이터는 군집의 결정 및 이탈 판정이 가능한 크기여야 하는데, 이를 위한 최소한의 데이터 날자 수.
  * 군집 유효 데이터 일 수 (e_d^g) 와 군집 이탈 판정일 수 (c_d^g) 의 합이다.
  * 예로, 유효 데이터 일 수 e_d = 28 이고 이탈 판정일 수 c^d = 14 이면 o_d^g 는 42 가 된다.
...
* *수집 윈도우 (col_window, w^g)* : 군집별 학습 데이터를 순회하며 데이터를 수집하는 윈도우.
  * 데이터 양을 늘리고 군집에 맞는 데이터 수집을 위해 이용
  * 군집별로 하나씩 할당
* *수집 윈도우 집합 (col_window_set, W)* : 학습 데이터 내 모든 군집별 수집 윈도우의 집합

$$W = \{ w^m : m \in \mathbb Z, k^{min} \leq m \leq U \}$$

* *수집 윈도우 시작일 (col_window_begin, w_b^g)*: : 특정 윈도우의 시작일. 슬라이딩되면서 윈도우 스텝만큼 증가.
* *수집 윈도우 크기 (col_window_width, w_w^g)* : 윈도우 크기는 해당 군집의 학습 데이터 일 수 (o_d^g) 와 같게 한다.

$$w_w^g = o_d^g$$

* *수집 윈도우 스텝 (col_window_step, w_s^g)* : 윈도우의 진행 스텝.
  * 중복을 허용하되, 적정 수준을 유지하도록 윈도우 크기에 대해 c (초모수)의 비율로 정한다.

$$ w_s^g = \lceil w_w^g \times c \rceil, \{c \in \mathbb R : 0 \lt c \leq 1 \}$$

* *수집 윈도우 종료일 (col_window_end, w_e^g)*: 윈도우 기간의 종료일
  * 수집 윈도우 시작일과 스텝으로 계산
  * 학습 데이터 종료일 (L_e) 이하여야 한다.

$$w_e^g = w_b^g + w_s^g, w_e^g \leq L_e$$

* 수집 윈도우 내 데이터는 크게 유효 데이터 일 수 (e_d^g) 와 이탈 판정일 수 (c_d^g) 로 나뉜다.
...
학습 데이터 내 모든 유효한 유저에 대해
  첫 접속일부터 데이터로 군집 (g) 을 결정
    군집의 슬라이딩 윈도우 (w^g) 진행
      군집에 속하면
        피처 데이터 (X) 를 군집 학습 데이터 (o) 에 넣음
        이탈 여부로 라벨링 (y)
      속하지 않으면
        현재 윈도우 시작일부터 데이터로 군집 (g) 결정
        속하는 군집이 없으면
          다음 유저로
```
...
예측 데이터 내 각 유저 데이터에 대해
  ...
  결석률 (a) 이 r_c 이상이면:
    기이탈로 판단해 제외
  유저의 단위 기간당 접속일 수 (k) 로 유저의 군집 결정
  선택된 군집의 모델로 예측
```
