...
* *유효 접속 일수 (effect_plays, e_p)* : 군집 판정, 학습이나 예측에서 몇 접속일의 데이터 포인트를 사용할지를 나타냄 (초모수).
...
* *유효 데이터 일수 (effect_data_days, e_d)* : 유저별로 유효 접속 일수에 해당하는 데이터 수집에 필요한 기간 일수.
  * 접속 주기 (f) 와 유효 접속 일수 (e_p) 를 곱해 추정할 수 있으나, 제 값은 유저별로 실제 접속일 데이터를 살펴보아야 알 수 있다.
...
* *접속 예정일 (due_play_day, q)* : 유효 데이터 기간 내 유저의 마지막 접속일 이후 예상되는 다음 접속일. 마지막 접속일 (l) 과 접속 주기 (f) 로 계산.

$$q = [l + f]$$

* *결석 일수 (absents, a)* : 접속 예정일 이후 중단 데이터 내에서 유저가 결석한 일수
  * 기간 내 접속 예정일부터 접속일에서 기간의 마지막 날까지 일수.
...
* *중단 확인 일수 (pause_test_days, p_d)* : 유저의 중단 확인에 필요한 날자 수. 중단 결석률 (r_p) 과 접속 주기 (f) 에서 계산
...
* *이탈 확인률 (churn_test_rate, r_t)* : 각 이탈 의심 유저에 대해, 이탈 확인에 필요한 일수를 계산하기 위한 비율. 중단 결석률에 대한 비율로, r_t = 1 이면 중단 결석률과 같은 일수 경과 후 이탈 판정을 하게 된다. (초모수)
  * 너무 작으면 제대로된 검증을 할 수 없고, 너무 크면 검증에 시간이 오래 걸리며 지나치게 엄격한 검증이 될 수 있다.

* *이탈 확인 일수 (churn_test_days, c_d)* : 각 중단 유저에 대해, 중단 판정일 후 이 일수까지의 데이터에 대해 이탈 확인을 수행한다. 유효 데이터에서 구한 접속 주기 (f) 에 중단 결석률 (r_p) 및 이탈 확인률 (r_t)을 곱해 구한다.
...
* *한계 학습 데이터 일수 (learn_data_margin_days, L_d^mar)* : 학습용 데이터는 군집 결정 및 이탈 확인이 가능해야 하는데, 이를 위해 필요한 최소한의 데이터 날자 수.
  ...
  * 예로, 최대 유효 데이터 일수 e_d^max = 28 이고 최대 중단 확인 일수 p_d^max = 14, 그리고 최대 이탈 확인 일수 c_d^max = 7 이면 한계 학습 데이터 일수 L_d^mar 은 49 가 된다.
...
필요한 경우 단위 기간 접속 일수 (k) 를 기준으로 유저의 군집을 나눌 수 있다. 이 경우 학습 데이터는 군집 별로 따로 준비한다:
...
학습 데이터 내 모든 유저에 대해
  ...
  수집 윈도우에 대해
    유효 접속 일수 (e_p) 에 해당하는 데이터 일수 (e_d) 구함
    유효 접속 일수분의 데이터를 찾을 수 없으면
      다음 유저로

    유효 데이터에서 단위 기간 접속 일수 (k) 와 접속 주기 (f) 구함
    ...
    중단 확인 데이터에서 접속 예정일 이후 결석 일수 (a) 구함
