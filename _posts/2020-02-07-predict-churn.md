...
* *이탈 검증 일수 (churn_test_days, t_d)* : 이탈 검증을 무한히 할 수는 없기에, 이 일수 동안만 검증한다. 이탈자의 이탈시 접속 주기 (f) 와 이탈 검증률을 곱해 정한다.
  * 예로, 어떤 이탈자의 이탈시 접속 주기가 f = 2.33 이고 이탈 검증률이 r_t = 6 이면, 이탈 검증 일수 t_d = 14 이다
  * 이 유저의 이탈 후 14일 동안 접속이 있으면 판정 오류, 접속이 없으면 판정이 맞은 것이다.

* *이탈 검증 기간 (churn_test_term)* : 이탈 판정 후 진짜로 이탈했는지 검증하는 기간. 각 이탈자 별로 이탈 판정일부터 이탈 검증 일수 이후까지이다.
