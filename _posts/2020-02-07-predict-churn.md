...
* 군집 분류:
  * g^a - 단위 기간당 접속일이 1, 2 일인 유저군
  * g^b - 단위 기간당 접속일이 3, 4 일인 유저군
...
* *군집 (group, g)* : 접속 빈도에 따른 학습 데이터와 예측 모델 분류에 사용. 군집은 유저의 단위 기간당 접속일 수 (k) 를 이용해, 서비스별 휴리스틱으로 정의한다. 예로, 앞에서 다음과 같이 군집을 설정하였다.
  * g^a - 단위 기간당 접속일이 1, 2 일인 유저군
  * g^b - 단위 기간당 접속일이 3, 4 일인 유저군
  * 이때, 단위 기간당 접속일이 1 일인 유저는 g^a, 3 일인 유저는 g^b 에 속하게 된다.
...
* *유효 데이터 일 수 (effect_data_days, e_d)* : 유저별로 유효 접속일 수에 해당하는 데이터 수집에 필요한 기간 일 수.
  ...
  * 예로, 접속 주기 f = 2.33 인 유저에 대해 유효 접속일 수 e_p = 4 로 데이터를 모으려면 e_d = 10 일 기간의 데이터를 확보해야 한다.

$$e_d = \lceil f \times e_p \rceil$$
...
예측 모델의 성패는 피처에 달려있다고 해도 과언이 아닐 것이다. 피처는 다양한 방식으로 발굴될 수 있는데, 이 글에서 다 다루기는 어렵기에 별도의 글 [온라인 게임을 위한 피처 유니버스 만들기]() 를 참고하도록 하자.

## 데이터 준비

앞서 말한 개념과 피처를 통해 학습 및 예측을 위한 데이터를 준비하게 된다. 중요한 것은 *모든 유저가 대상은 아니라는 점* 이다. 게임 가입 후 적응하지 못하여 초기에 이탈했거나, 비정기적으로 플레이하는 유저, 그리고 어뷰징 툴을 사용하는 등의 유저는 대상으로 하지 않는 것이 *기대 이익을 최대화하는 이탈 검출* 을 위해 바람직할 것이다.
...
* *최소 학습 데이터 일 수 (min_learn_data_days, L_d^min)* : 학습 용 데이터는 군집 결정 및 이탈 판정이 가능해야하는데, 이를 위해 필요한 최소한의 데이터 날자 수.
  ...
  * 예로, 최대 유효 데이터 일 수 e_d^max = 28 이고 최대 이탈 판정일 수 c_d^max = 14 이면 L_d^min 은 42 가 된다.

$$L_d^{min} = e_d^{max} + c^{max}$$

> 최소 학습 데이터 일 수는 어디까지나 통계적인 값이다. 이에 따라 데이터를 확보해도 그 기간동안 유저 행동에 변동이 있으면 데이터가 부족해질 수 있기에, 실제 학습 데이터 일 수는 최소 학습 데이터 일 수 보다 여유있게 잡자.
...
앞에서 단위 기간당 접속일 수 (k) 로 유저의 군집을 배정하는 것을 보았다. 이제 학습 데이터에서 *군집 학습 데이터 집합 (group_learn_data_set, O)* 을 다음과 같이 준비한다:

* 군집 집합 (G) 내 각 군집에 대해 각각의 군집 학습 데이터 (group_learn_data, o^k) 로 구성

$$O = \{ o^k \vert k \in \mathbb Z, k^{mar} \leq k \leq U\}$$

* 예로, 단위 기간 U = 7 이고 단위 기간 한계 접속일 수 k^mar = 3 인 경우 군집 학습 데이터 집합은 다음과 같다:

$$ O = \{ o^3, o^4, o^5, o^6, o^7 \}$$

* 같은 유저도 시간 경과에 따라 접속 주기가 달라질 수 있기에, 이어 설명할 *수집 윈도우* 를 이용해 시기별 학습 데이터를 적절한 군집에 배정한다.
* 예 : 어떤 유저가 수집 윈도우에서 g^5 -> g^3 -> g^1 순으로 군집이 변하다 이탈
...
* 수집 윈도우 내 데이터는 유효 데이터 일 수 (e_d) 의 데이터 + 이탈 판정일 수 (c_d)의 데이터로 구성된다.
...
* 수집 윈도우는 스텝 단위로 슬라이딩하다가 윈도우 종료일이 학습 데이터 종료일보다 커질 때 끝난다.
...
학습 데이터 내 모든 유저에 대해
  ...
  수집 윈도우에 대해
    ...
    수집 윈도우 종료일이 학습 데이터 종료일보다 크면
      다음 유저로
    윈도우의 단위 기간당 접속일 수 (k) 로 군집 (g^k) 결정 후, 윈도우의 피처 데이터 (X) 를 군집 학습 데이터 (o^k) 에 추가
    이탈 판정일 수 동안 접속일로 이탈 여부 라벨링 (y) 후 군집 학습 데이터에 추가
    이탈했으면
      다음 유저로
    수집 윈도우 스텝으로 윈도우 진행
```
...
* *모델 집합 (model set, M)* : 각 군집별 모델의 집합

$$M = \{ m^k \vert k \in \mathbb Z, k^{mar} \leq k \leq U\}$$

* 예로, 단위 기간 U = 7 이고 단위 기간 한계 접속일 수 k^mar = 3 인 경우 모델 집합은 다음과 같다:
