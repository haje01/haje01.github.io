...
학습 데이터는 이후 설명할 수집 윈도우를 통해 유효 데이터, 이탈 확인 데이터의 두 부분으로 나누어 수집한다. 먼저 관련 용어를 살펴보자.

* *이탈 확인 데이터 (churn_test_data)* : 앞에서 설명한 유효 데이터 일수(e_d) 이후 아래에서 설명할 이탈 확인 일수 (c_d) 만큼의 데이터로, 이탈 여부 판단에 사용된다.

* *이탈 확인률 (churn_test_rate, c_r)* : 각 유저에 대해, 이탈 확인에 필요한 일수를 계산하기 위한 값. 접속 주기 (f) 에 대한 비율로, 오탐이나 미탐이 적도록 설정 (초모수)
...
* *이탈 확인 일수 (churn_test_days, c_d)* : 각 유저에 대해, 이탈 확인에 필요한 날자 수.
  ...
  * 예로 접속 주기 f = 1.75 인 유저에 대해 이탈 확인률 c_r = 6 으로 하면 이탈 확인 일수 c_d = 11 로, 유효 데이터 이후 11 일의 데이터가 있어야 이탈 판단이 가능하다.
...
* *수집 윈도우 종료일 (col_window_end, w_e)* : 수집 윈도우 시작일부터 수집 윈도우 크기로 계산

$$w_e = w_b + w_w$$

여기까지의 설명을 그림으로 나타내면 아래와 같다:

![수집 윈도우](/assets/2020-08-07-10-43-14.png)
...
학습 데이터 내 모든 유저에 대해
  ...
  수집 윈도우에 대해
    ...
    이탈 확인 데이터가 부족하면
      다음 유저로
    이탈 확인 데이터에서 구한 단위 기간 접속 일수 (k) 가 단위 기간 한계 접속 일수 (k^mar) 미만이면
      이탈 판정
...
![수집 과정](/assets/2020-08-07-10-42-20.png)
