...
* *휴식 확인 일수 (idle_test_days, i_d)* : 유저의 휴식 확인에 필요한 날자 수.
  ...
  * 이 기간내 접속일 수가 단위 기간 한계 접속 일수 (k^mar) 미만이면 휴식 (idle) 으로 판정한다.
...
* *이탈 확인 일수 (churn_test_days, c_d)* : 각 휴식 유저에 대해, 이탈 확인에 필요한 날자 수. 휴식 판정일 후 이 일수까지의 데이터에 대해 이탈 확인을 수행한다.
  ...
  * 이 기간내 접속일 수가 단위 기간 한계 접속 일수 (k^mar) 미만이면 이탈 (churn)로 판정하고, 아니면 휴식 (idle) 을 유지한다.
...
유저별 예측 후 실제 모델의 성능 평가를 위해 검증이 꼭 필요하다. 검증은 예측 후 데이터에 대해 학습 데이터와 같은 방식으로 활성, 휴식, 이탈을 판정한다. 다만, 윈도우를 진행하지 않고 한 번만 판정한다.

예측한 모든 유저에 대해 검증하려면 충분한 시간이 흐른뒤 검증을 실시해야 한다. 이 과정에서 예측 모델의 성능이 떨어졌으면, 예측 실패한 사례를 포함한 최신 데이터로 다시 학습을 진행해야 한다.

#### 용어와 개념

* *한계 검증 데이터 일수 (verify_data_margin_days, V_d^max)* : 예측한 모든 유저를 검증하기 위해 필요한 최소한의 데이터 일수
  * 최대 휴식 확인 일수 (i_d^max) 와 최대 이탈 확인 일수 (c_d^max) 의 합으로 계산한다.
  * 예측일 이후 이 일수의 데이터로 검증 한다.

$$V_d^{max} = i_d^{max} + c_d^{max}$$
...
예측 결과 내 모든 유저에 대해
  예측 시점의 접속 주기 (f) 를 얻고, 이를 이용해 휴식 확인 일수 (i_d) 를 계산
  휴식 확인 데이터내 단위 기간 접속 일수 (k) 가 단위 기간 한계 접속 일수 (k^mar) 보다 작으면
      휴식 판정
  휴식 상태이면
    이탈 확인 일수 (c_d) 계산
    이탈 확인 데이터내 접속일이 1일 미만이면
      이탈 판정
  예측 결과와 판정을 비교
검증 결과 집계
```
...
* *미숙 확인 일수 (unripe_test_days, u_d)* : 유저의 가입 이후 미숙 여부 확인에 필요한 날자 수 (초모수)
  ...
  * 예를 들어 단위 기간 한계 접속 일수 k^mar = 3 이고, 미숙 확인 일수 u_d = 4, 한계 평균 이용 시간 h^mar = 0.25 (15분) 일때,
    * 어떤 유저의 가입 후 4 일 동안 접속일이 하루이고, 그때 이용 시간이 10 분 이었다면
    * 이 유저의 단위 기간 접속 일수 k = 1.75, 평균 이용 시간 h = 0.17 이 되고,
    * k (= 1.75) < k^mar (= 3) 및 h (=0.17) < h^mar (=0.25) 에서 이 유저는 미숙으로 판정된다.
