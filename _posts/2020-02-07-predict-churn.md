...
* *단위 기간 한계 접속 일수 (unit_margin_play, k^mar)* : 단위 기간 접속 일수 (k) 의 한계값. 유저의 단위 기간 접속일수가 이 값 미만이면 학습 및 예측에서 제외한다 (초모수).
...
* *접속 주기 (play_freq, f)* : 유저가 몇 일마다 접속하는지의 값. 유저가 대상 기간내 접속한 첫 날과 마지막 날로 유저별 기간을 정한뒤, 그 기간 내 기간 일수 (d) 와 접속 일수 (p) 로 계산
  * 예로, 기간 일수 d = 7 동안 접속 일수 p = 4 인 경우 접속 주기 f = 1.75
...
* *결석률 (absent_rate, r)* : 유저의 접속 예정일 (q) 이후, 예측일 (t) 까지 결석 일수를 접속 주기에 대한 비율로 나타낸 것 (0 이상).
...
* *한계 학습 데이터 일수 (learn_data_margin_days, L_d^mar)* : 학습용 데이터는 군집 결정 및 이탈 판정이 가능해야하는데, 이를 위해 필요한 최소한의 데이터 날자 수.
  ...
  * 예로, 최대 유효 데이터 일수 e_d^max = 28 이고 최대 이탈 판정 일수 c_d^max = 14 이면 L_d^mar 은 42 가 된다.
...
학습 데이터 내 모든 유저에 대해
  ...
  수집 윈도우에 대해
    ...
    윈도우의 유효 데이터 일수 (e_d) 의 피처 데이터 (X) 를 군집의 학습 데이터에 추가
    이탈 판정 일수 (c_d) 동안 접속일로 이탈 여부 (y) 를 군집의 학습 데이터에 추가
    이탈했으면
      다음 유저로
...
이탈로 판정하여 라벨링한 유저가 며칠 후 다시 접속하는 경우가 많다면 학습 데이터가 부정확해지는 문제가 발생한다. 일부 유저만의 예외적인 경우이거나 캠페인의 결과로 회귀한 경우일 수 있겠으나, 일단 초모수인 이탈 결석률 (r_c) 이 너무 작지 않은가 확인이 필요하다. 아래의 이탈 검증률 (r_t) fmf dldydgo 이탈 판정을 지속적으로 검증하고, 필요시 이탈 결석률을 튜닝하도록 하자.

* *이탈 검증률 (churn_test_rate, r_t)* : 이탈 판정 후 유저의 결석률이 이 값보다 클 때까지 이탈을 검증. 검증 기간동안 방문이 있으면 판정 오류 처리 (초모수)
  * 예로, r_t = 6 이면 여섯 번의 접속 주기만큼 결석하면 더 이상 검증하지 않음.
